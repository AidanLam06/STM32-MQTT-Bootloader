...
int main(void)
{
  ...
  HAL_StatusTypeDef status = HAL_UART_Receive(&huart1, rx_buffer, 1, 5000);

  if (status == HAL_OK && rx_buffer[0] == 0xAA) {
	  HAL_UART_Transmit(&huart2, (uint8_t *)"Received UART Traffic\r\n", 25, 100);
	  while (__HAL_UART_GET_FLAG(&huart2, UART_FLAG_TC) == RESET);
	  //crc check would prolly go here
	  uint8_t ack = 0x79;
	  HAL_UART_Transmit(&huart1, &ack, 1, 100);
	  // add update mode code below this comment
  } else {
      if (status == HAL_TIMEOUT) {
          HAL_UART_Transmit(&huart2, (uint8_t *)"Error: Real Timeout\r\n", 21, 100);
      } else if (status == HAL_ERROR) {
          HAL_UART_Transmit(&huart2, (uint8_t *)"Error: Hardware/HAL Error\r\n", 27, 100);
      } else if (status == HAL_BUSY) {
          HAL_UART_Transmit(&huart2, (uint8_t *)"Error: Peripheral Busy\r\n", 24, 100);
      }
      while (__HAL_UART_GET_FLAG(&huart2, UART_FLAG_TC) == RESET);
      jumpToApplication();
  }
